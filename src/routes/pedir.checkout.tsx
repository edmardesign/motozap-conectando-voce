...
export const Route = createFileRoute("/pedir/checkout")({ component: Checkout });
...
function Checkout() {
  ...
  const { itens, subtotal, lojaId, limpar } = useCarrinho();
  ...
  async function finalizar() {
    ...
    try {
      ...
      const { error: e2 } = await supabase.from("food_pedido_itens").insert(itensPayload);
      if (e2) throw e2;

      limpar();
      navigate({ to: "/pedir/confirmado/$id", params: { id: pedido.id } });
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)", minHeight: "100dvh" }}>
      <header className="flex items-center gap-3 p-4" style={{ background: BG, borderBottom: `1px solid ${BORDER}` }}>
        <Link to="/pedir/carrinho" className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: CARD_3 }}>
