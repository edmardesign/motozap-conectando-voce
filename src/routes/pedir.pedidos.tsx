...
export const Route = createFileRoute("/pedir/pedidos")({ component: Pedidos });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from("food_pedidos")
        .select("id, numero_pedido, status, total, created_at, food_lojas(nome), food_pedido_itens(quantidade)")
        .eq("cliente_user_id", user.id)
        .order("created_at", { ascending: false })
        .limit(50);
      const mapped: PedidoLinha[] = (data ?? []).map((p) => {
        const loja = (p as unknown as { food_lojas: { nome: string } | null }).food_lojas;
        const its = (p as unknown as { food_pedido_itens: { quantidade: number }[] }).food_pedido_itens ?? [];
        return {
          id: p.id,
