...
import { useCarrinho, formatBRL } from "@/lib/pedir/carrinho";
import { supabase } from "@/integrations/supabase/client";
import { mapLoja } from "@/lib/pedir/db";
import type { Loja } from "@/lib/pedir/tipos";

export const Route = createFileRoute("/pedir/carrinho")({ component: Carrinho });
...
function Carrinho() {
  const { itens, subtotal, ajustar, remover, lojaId } = useCarrinho();
  const navigate = useNavigate();
  const [loja, setLoja] = useState<Loja | null>(null);

  useEffect(() => {
    if (!lojaId) { setLoja(null); return; }
    supabase.from("food_lojas").select("*").eq("id", lojaId).maybeSingle().then(({ data }) => {
    ...
    return (
      <div style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)", minHeight: "100dvh" }}>
        ...
        <div className="p-8 text-center">
          ...
          <Link to="/pedir" className="inline-block px-5 py-3 rounded-full font-black text-sm" style={{ background: BRAND, color: BRAND_INK }}>
            Ver restaurantes
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)", minHeight: "100dvh" }}>
      ...
      <main className="p-4 flex flex-col gap-4">
        ...
        <button
          onClick={() => navigate({ to: "/pedir/checkout" })}
...
function Header() {
  return (
    <header className="flex items-center gap-3 p-4" style={{ background: BG, borderBottom: `1px solid ${BORDER}` }}>
      <Link to="/pedir" className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: CARD_3 }}>
