...
import { imprimirComanda } from "@/lib/mercado/comanda";
import type { Database } from "@/integrations/supabase/types";


export const Route = createFileRoute("/estabelecimento/mercado")({
  beforeLoad: () => {
    throw redirect({ to: "/parceiros/mercado" });
  },
});
...
type StatusPedido = Database["public"]["Enums"]["mercado_status_pedido"];
type Loja = Database["public"]["Tables"]["mercado_lojas"]["Row"];
type Produto = Database["public"]["Tables"]["mercado_produtos"]["Row"];
type PedidoRow = Database["public"]["Tables"]["mercado_pedidos"]["Row"];
type ItemRow = Database["public"]["Tables"]["mercado_pedido_itens"]["Row"];
...
export function PainelMercado() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      toast.error("Você precisa estar logado como mercado");
    ...
    (async () => {
      ...
      if (error) {
        toast.error("Não foi possível carregar seus mercados, tente novamente");
...
function PainelLoja({
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "mercado_pedidos", filter: `loja_id=eq.${loja.id}` },
        (payload) => {
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "mercado_pedido_itens" },
        () => recarregarPedidos()
      )
      .subscribe();
    return () => {
  ...
  async function togglePausado() {
    const novo = !pausado;
    setPausado(novo);
    const { error } = await supabase.from("mercado_lojas").update({ pausado: novo }).eq("id", loja.id);
    if (error) {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#000", color: "#fff" }}>
      ...
      <header className="flex items-center gap-3 p-4 sticky top-0 z-10" style={{ background: "#0A0A0A", borderBottom: `1px solid ${NEON}33` }}>
        <Link to="/mercado" className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "#1a1a1a" }}>
          <ArrowLeft size={18} />
        </Link>
...
function AbaCardapio({
  ...
  async function excluir(p: Produto) {
    ...
    const { error } = await supabase.from("mercado_produtos").delete().eq("id", p.id);
    if (error) {
  ...
  async function salvar() {
    ...
    const { error } = form.id
      ? await supabase.from("mercado_produtos").update(payload).eq("id", form.id)
      : await supabase.from("mercado_produtos").insert(payload);
    if (error) {
...
function AbaCupons({ lojaId }: { lojaId: string }) {
  ...
  async function salvar() {
    ...
    const q = form.id
      ? (supabase as any).from("mercado_cupons").update(payload).eq("id", form.id)
      : (supabase as any).from("mercado_cupons").insert(payload);
    const { error } = await q;
  ...
  async function toggleAtivo(c: Cupom) {
    const { error } = await (supabase as any).from("mercado_cupons").update({ ativo: !c.ativo }).eq("id", c.id);
  ...
  async function excluir(c: Cupom) {
    ...
    const { error } = await (supabase as any).from("mercado_cupons").delete().eq("id", c.id);
...
function AbaHorarios({ lojaId }: { lojaId: string }) {
  ...
  async function carregar() {
    ...
    const { data } = await supabase
      .from("mercado_horarios" as never)
      .select("*")
      .eq("loja_id", lojaId)
  ...
  async function salvarTudo() {
    ...
    for (const h of linhas) {
      if (h.id) {
        await (supabase as any).from("mercado_horarios")
...
function AbaAcertos({ lojaId }: { lojaId: string }) {
  ...
  async function calcular() {
    setCarregando(true);
    const { data, error } = await (supabase as any).rpc("mercado_relatorio_acerto", { _loja_id: lojaId, _desde: desde, _ate: ate });
  ...
  async function fechar() {
    ...
    setFechando(true);
    const { error } = await (supabase as any).rpc("mercado_fechar_acerto", { _loja_id: lojaId, _desde: desde, _ate: ate });
    setFechando(false);
