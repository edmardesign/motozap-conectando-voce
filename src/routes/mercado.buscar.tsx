...
export const Route = createFileRoute("/mercado/buscar")({ component: Buscar });
        ...
        if (debounced === "") {
          ...
          const [{ data: dl, error: e1 }, { data: dp, error: e2 }] = await Promise.all([
            ...
            supabase
              ...
              .select("*, mercado_lojas!inner(nome, ativo)")
              .eq("disponivel", true)
              .eq("mercado_lojas.ativo", true)
              .ilike("nome", `%${debounced}%`)
              .limit(20),
          ]);
          ...
          setProdutos(
            (dp ?? []).map((r) => ({
              ...
              lojaNome: (r as { mercado_lojas: { nome: string } }).mercado_lojas.nome,
            }))
          );
        }
      } catch {
  ...
  return (
    <div>
      <header className="sticky top-0 z-30 p-3 flex items-center gap-2" style={{ background: "var(--dz-bg)", borderBottom: "1px solid var(--dz-line)" }}>
        <Link to="/mercado" className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "var(--dz-surface-2)" }}>
      ...
      <main className="p-4 flex flex-col gap-6">
        ...
        <section>
          <h3 className="dz-h2 mb-2">Mercados</h3>
          <ul className="flex flex-col gap-2">
            {lojas.map((l) => (
              <li key={l.id}>
                <Link
