...
export const Route = createFileRoute("/mercado/acompanhar/$id")({ component: Acompanhar });
...
function Acompanhar() {
  const { id } = useParams({ from: "/mercado/acompanhar/$id" });
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "mercado_pedidos", filter: `id=eq.${id}` },
        (payload) => {
  ...
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)", minHeight: "100dvh" }}>
      <header className="flex items-center gap-3 p-4" style={{ background: BG, borderBottom: `1px solid ${BORDER}` }}>
        <Link to="/mercado/pedidos" className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: CARD_3 }}>
