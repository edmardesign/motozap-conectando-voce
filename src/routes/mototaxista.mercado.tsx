...
export const Route = createFileRoute("/mototaxista/mercado")({
  head: () => ({
    meta: [
      { title: "Entregas Mercado — Bora Zé! Mototaxista" },
      { name: "description", content: "Aceite pedidos de mercado prontos para entrega." },
    ],
  }),
  component: MototaxistaMercado,
});
...
type Pedido = {
  ...
  mercado_lojas?: { nome: string; endereco: string | null; bairro: string | null } | null;
};

function MototaxistaMercado() {
  ...
  const loadPedidos = useCallback(async () => {
    ...
    const [disp, meus] = await Promise.all([
      supabase
        ...
        .select("id,numero_pedido,loja_id,status,endereco_entrega,bairro_entrega,taxa_entrega,total,pronto_em,entregue_em,entregador_user_id,mercado_lojas(nome,endereco,bairro)")
        .eq("status", "pronto")
  ...
  useEffect(() => {
    ...
    loadPedidos();
    const ch = supabase
      .channel("mercado-mototaxista")
      .on("postgres_changes", { event: "*", schema: "public", table: "mercado_pedidos" }, () => loadPedidos())
      .subscribe();
    return () => {
  ...
  async function aceitar(id: string) {
    ...
    setBusy(id);
    const { error } = await supabase.rpc("mototaxista_aceitar_pedido_mercado", { _pedido_id: id });
    setBusy(null);
    if (error) {
  ...
  async function finalizar(id: string) {
    setBusy(id);
    const { error } = await supabase.rpc("mototaxista_marcar_entregue_mercado", { _pedido_id: id });
    setBusy(null);
    if (error) {
      toast.error(error.message);
      return;
    ...
    <main className="min-h-screen text-white" style={{ background: "#000", paddingBottom: 24 }}>
      ...
      >
        ...
        <div className="flex-1">
          <h1 className="text-base font-black">Entregas Mercado</h1>
          ...
          <article key={p.id} className="rounded-xl p-4 space-y-3" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-[14px] font-bold">{p.mercado_lojas?.nome ?? "Loja"}</div>
            ...
            <div className="space-y-1.5 text-[13px]">
              {p.mercado_lojas?.endereco && (
                <div className="flex gap-2">
                  <Store size={14} className="mt-0.5 shrink-0" style={{ color: DIM }} />
                  <span>
                    Retirar: {p.mercado_lojas.endereco}
                    {p.mercado_lojas.bairro ? ` • ${p.mercado_lojas.bairro}` : ""}
                  </span>
                </div>
              )}
              <div className="flex gap-2">
