...
export const Route = createFileRoute("/mercado/confirmado/$id")({ component: Confirmado });
...
function Confirmado() {
  const { id } = useParams({ from: "/mercado/confirmado/$id" });
  const [r, setR] = useState<Resumo | null>(null);

  useEffect(() => {
    supabase
      .from("mercado_pedidos")
      .select("numero_pedido, total, endereco_entrega, mercado_lojas(nome)")
      .eq("id", id)
      .maybeSingle()
      .then(({ data }) => {
        if (!data) return;
        const loja = (data as { mercado_lojas: { nome: string } | null }).mercado_lojas;
        setR({
  ...
  return (
    <div className="min-h-[80dvh] flex flex-col items-center justify-center p-6 text-center" style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)" }}>
      ...
      <div className="w-full max-w-sm mt-8 flex flex-col gap-3">
        <Link to="/mercado/acompanhar/$id" params={{ id }} className="rounded-full px-5 py-4 font-black text-sm" style={{ background: BRAND, color: BRAND_INK, boxShadow: "var(--dz-shadow-2)" }}>
          Acompanhar entrega
        </Link>
        <Link to="/mercado" className="text-center text-sm underline" style={{ color: MUTED }}>Voltar ao início</Link>
      </div>
    </div>
  );
}
