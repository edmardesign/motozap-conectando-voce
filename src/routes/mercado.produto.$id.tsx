...
import { mapProduto } from "@/lib/mercado/db";
import type { Produto, Adicional } from "@/lib/mercado/tipos";
import { formatBRL, useCarrinhoMercado } from "@/lib/mercado/carrinho";
import { toast } from "sonner";

export const Route = createFileRoute("/mercado/produto/$id")({ component: ProdutoPage });
...
function ProdutoPage() {
  const { id } = useParams({ from: "/mercado/produto/$id" });
  ...
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)" }}>
      <div className="relative h-56">
        ...
        <Link to="/mercado/loja/$id" params={{ id: p.lojaId }} className="absolute top-3 left-3 w-10 h-10 rounded-full flex items-center justify-center" style={{ background: CARD, border: `1px solid ${BORDER}`, boxShadow: "var(--dz-shadow-1)" }}>
      ...
      <div className="fixed left-0 right-0 bottom-0 p-4 pb-20 z-30" style={{ background: `linear-gradient(to top, ${BG} 60%, transparent)` }}>
        <div className="flex gap-3 items-center max-w-[460px] mx-auto">
          ...
          <button
            onClick={() => { adicionar(p, qtd, adicionais, obs || undefined); navigate({ to: "/mercado/carrinho" }); }}
