...
import { formatBRL, useCarrinhoMercado } from "@/lib/mercado/carrinho";
import { mapLoja, mapProduto } from "@/lib/mercado/db";
import type { Loja, Produto } from "@/lib/mercado/tipos";
import { useAuth } from "@/hooks/use-auth";
import bannerAraci from "@/assets/mercado/banner-araci.png.asset.json";
import bannerCestao from "@/assets/mercado/banner-cestao.png.asset.json";
import bannerBebeFrios from "@/assets/mercado/banner-bebe-frios.png.asset.json";
import bannerParcele from "@/assets/mercado/banner-parcele.png.asset.json";
import reel1 from "@/assets/mercado/reel-1.mp4.asset.json";
import reel2 from "@/assets/mercado/reel-2.mp4.asset.json";
import reel3 from "@/assets/mercado/reel-3.mp4.asset.json";
import reel4 from "@/assets/mercado/reel-4.mp4.asset.json";
import reel5 from "@/assets/mercado/reel-5.mp4.asset.json";
import { MercadoOnboarding } from "@/components/mercado/onboarding";

const BANNERS = [
  { src: bannerAraci.url, alt: "Chegamos em Araci" },
  { src: bannerCestao.url, alt: "Cestão Supermercado — os melhores produtos" },
  { src: bannerBebeFrios.url, alt: "Bebé Frios — promoção os melhores frios" },
  { src: bannerParcele.url, alt: "Parcele suas compras no cartão em até 12x" },
];
...
function Home() {
  const { totalItens, subtotal } = useCarrinhoMercado();
  const { user } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (lojasErr) {
        toast.error("Não foi possível carregar os mercados, tente novamente");
        setCarregando(false);
        return;
      }
  ...
  return (
    <div style={{ background: "var(--dz-bg)" }}>
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-[19px] font-black text-[color:var(--dz-ink)] leading-tight">
            Mercado Bora Zé! <span aria-hidden>👋</span>
          </h1>
      ...
      <main className="flex flex-col gap-7 pb-4">
        ...
        <section className="px-5">
          <div className="grid grid-cols-5 gap-2.5">
            ...
            <Link
              ...
              className="flex items-center justify-center rounded-2xl py-3 transition-transform active:scale-[0.97]"
              style={{ background: "var(--dz-surface-2)" }}
            >
        ...
        <section>
          <div className="dz-section-title">
            <h2 className="dz-h1">🛒 Todos os mercados</h2>
          ...
          <ul className="flex flex-col gap-3 px-5">
            ...
            {!carregando && lojas.length === 0 && (
              <li className="dz-caption text-center py-6">Nenhum mercado encontrado.</li>
            )}
          </ul>
        </section>
...
function BannersCarrossel() {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      if (!child) return;
      // rolagem suave até o próximo slide
  ...
  // sincroniza índice quando o usuário arrasta manualmente
  function onScroll() {
    const node = scrollerRef.current;
    if (!node) return;
