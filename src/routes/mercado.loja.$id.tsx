...
export const Route = createFileRoute("/mercado/loja/$id")({ component: LojaPage });
...
function LojaPage() {
  const { id } = useParams({ from: "/mercado/loja/$id" });
    ...
    (async () => {
      ...
      try {
        const [{ data: lojaRow, error: eL }, { data: catRows }, { data: prodRows, error: eP }] =
          await Promise.all([
            supabase.from("mercado_lojas").select("*").eq("id", id).maybeSingle(),
            supabase
              .from("mercado_categorias_cardapio")
              .select("*")
              .eq("loja_id", id)
              .eq("ativa", true)
        ...
        toast.error("Não foi possível carregar o mercado, tente novamente");
      } finally {
        setCarregando(false);
      }
    })();
  ...
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: "var(--dz-font)" }}>
      ...
      >
        <Link to="/mercado" className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: CARD, border: `1px solid ${BORDER_STRONG}`, boxShadow: "var(--dz-shadow-1)" }}>
      ...
      <main className="px-4 pt-5 pb-6">
        ...
        {produtos.length === 0 && (
          <p className="text-[13px]" style={{ color: MUTED }}>Este mercado ainda não cadastrou itens.</p>
        )}
      </main>

      {totalItens > 0 && !aberto && (
        <Link to="/mercado/carrinho" className="fixed left-4 right-4 z-30 flex items-center justify-between rounded-full px-5 py-3.5" style={{ bottom: 84, background: BRAND, color: BRAND_INK, boxShadow: "var(--dz-shadow-3)", maxWidth: 460, margin: "0 auto" }}>
...
function ProdutoModal({ produto, onClose }: { produto: Produto; onClose: () => void }) {
  ...
  const { adicionar } = useCarrinhoMercado();
