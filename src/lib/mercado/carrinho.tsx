...
const STORAGE_KEY = "boraze:mercado:carrinho:v1";
...
export function CarrinhoMercadoProvider({ children }: { children: ReactNode }) {
...
export function useCarrinhoMercado() {
  const ctx = useContext(CarrinhoCtx);
  if (!ctx) throw new Error("useCarrinhoMercado fora do CarrinhoMercadoProvider");
  return ctx;
}
