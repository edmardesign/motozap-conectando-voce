  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col" style={{ background: "#FFFFFF" }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
        <button onClick={onClose} className="text-foreground text-2xl leading-none">←</button>
        {outro.foto_url ? (
        ...
        {telefoneLimpo && (
          <a href={`tel:+55${telefoneLimpo}`} className="text-foreground text-xl px-2" aria-label="Ligar"><EmojiIcon e="📞" /></a>
        )}
      ...
      {/* Mensagens */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-4 space-y-2" style={{ background: "#FFFFFF" }}>
        {msgs.length === 0 && (
          <div className="text-center text-foreground/40 text-[13px] mt-8">
        ...
        {msgs.map((m) => {
          ...
          if (sistema) {
            return (
              <div key={m.id} className="text-center text-[12px] text-muted-foreground py-1">
                {m.texto}
              </div>
            );
          }
      ...
      {/* Composer */}
      <div className="px-3 py-2 flex items-end gap-2" style={{ background: "#FFFFFF" }}>
        <button
